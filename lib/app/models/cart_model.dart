...
class Items {
  ...
  Items.fromJson(Map<String, dynamic> json) {
    ...
    // Extracting data from 'webinar' object
    if (json['webinar'] != null) {
      image = json['webinar']['image'];
      title = json['webinar']['title'];
      price = double.parse(json['webinar']['price']?.toString() ?? '0').toInt();
      discount = json['webinar']['discount'];
    }

    // Extracting data from 'user' object
    if (json['user'] != null) {
      teacherName = json['user']['full_name'];
      rate = json['user']['rate']?.toString();
    }
